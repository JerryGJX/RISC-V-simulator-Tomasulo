...
#ifndef RISC_V_TOMASULO__CPU_HPP_
#define RISC_V_TOMASULO__CPU_HPP_
#include "memory.hpp"
#include "Decoder.hpp"
#include "iostream"
#include "Clock.hpp"
...
class CPU {
  ...
  ROB rob_prev{}, rob_next{};
  storeLoadBuffer slb_prev{}, slb_next{};
  fetchQueue fq_prev{}, fq_next{};
  RAM<> ram{};
  Decoder decoder{};
  Clock time{};
  IntALU int_alu;
...
//-----------------------
  ...
  //----------------------
  Wire<Null> issue_to_fetch;
  Wire<IssueToRegfile> issue_to_regfile;
  Wire<IssueToRob> issue_to_rob;
  Wire<IssueToRs> issue_to_rs;
  Wire<IssueToSlb> issue_to_slb;
  //-----------------
  Wire<RsToEx> rs_to_ex;
  //--------------------
  Wire<CommitToRegfile> commit_to_regfile;
  Wire<CommitToSlb> commit_to_slb;
  //---------------------
  Wire<SlbToRs> slb_to_rs_prev, slb_to_rs_next;
  Wire<SlbToSlb> slb_to_slb_prev, slb_to_slb_next;
  Wire<SlbToRob> slb_to_rob_prev, slb_to_rob_next;
  //----------------------
  Wire<ExToRob> ex_to_rob;
  Wire<ExToSlb> ex_to_slb;
  Wire<ExToRs> ex_to_rs;
  //---------------------------
  Wire<RobToCommit> rob_to_commit;

  //------------------------------------------------
  bool if_clear_fq = false;
  bool if_clear_rob = false;
  bool if_clear_rs = false;
  bool if_clear_slb = false;
  ...
  void PrintStall() {
    ...
    std::cout << "fetch_to_issue.stall:    " << "\t" << ((fetch_to_issue.stall) ? "true" : "false") << std::endl;
    //----------------------
    std::cout << "issue_to_fetch.stall:    " << "\t" << ((issue_to_fetch.stall) ? "true" : "false") << std::endl;
    std::cout << "issue_to_regfile.stall:  " << "\t" << ((issue_to_regfile.stall) ? "true" : "false") << std::endl;
    std::cout << "issue_to_rob.stall:      " << "\t" << ((issue_to_rob.stall) ? "true" : "false") << std::endl;
    std::cout << "issue_to_rs.stall:       " << "\t" << ((issue_to_rs.stall) ? "true" : "false") << std::endl;
    std::cout << "issue_to_slb.stall:      " << "\t" << ((issue_to_slb.stall) ? "true" : "false") << std::endl;

    //-----------------
    std::cout << "rs_to_ex.stall:          " << "\t" << ((rs_to_ex.stall) ? "true" : "false") << std::endl;

    //--------------------
    std::cout << "commit_to_regfile.stall: " << "\t" << ((commit_to_regfile.stall) ? "true" : "false") << std::endl;
    std::cout << "commit_to_slb.stall:     " << "\t" << ((commit_to_slb.stall) ? "true" : "false") << std::endl;

    //---------------------
    std::cout << "slb_to_rs_prev.stall:    " << "\t" << ((slb_to_rs_prev.stall) ? "true" : "false") << std::endl;
    std::cout << "slb_to_rs_next.stall:    " << "\t" << ((slb_to_rs_next.stall) ? "true" : "false") << std::endl;

    std::cout << "slb_to_slb_prev.stall:   " << "\t" << ((slb_to_slb_prev.stall) ? "true" : "false") << std::endl;
    std::cout << "slb_to_slb_next.stall:   " << "\t" << ((slb_to_slb_next.stall) ? "true" : "false") << std::endl;

    std::cout << "slb_to_rob_prev.stall:   " << "\t" << ((slb_to_rob_prev.stall) ? "true" : "false") << std::endl;
    std::cout << "slb_to_rob_next.stall:   " << "\t" << ((slb_to_rob_next.stall) ? "true" : "false") << std::endl;

    //----------------------
    std::cout << "ex_to_rob.stall:         " << "\t" << ((ex_to_rob.stall) ? "true" : "false") << std::endl;
    std::cout << "ex_to_slb.stall:         " << "\t" << ((ex_to_slb.stall) ? "true" : "false") << std::endl;
    std::cout << "ex_to_rs.stall:          " << "\t" << ((ex_to_rs.stall) ? "true" : "false") << std::endl;

    //---------------------------
    std::cout << "rob_to_commit.stall:     " << "\t" << ((rob_to_commit.stall) ? "true" : "false") << std::endl;
 ...
 private:
  ...
  //mem,pc_fetch,regfile,inst_fetch_and_queue,issue
  //reservation,ex,slbuffer,rob,commit
 public:
...
#endif //RISC_V_TOMASULO__CPU_HPP_
